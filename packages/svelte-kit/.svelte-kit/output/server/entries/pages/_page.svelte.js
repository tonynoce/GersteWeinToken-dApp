import { c as create_ssr_component, a as subscribe, d as is_promise, n as noop, e as escape, f as set_store_value, v as validate_component, h as add_attribute } from "../../chunks/index.js";
...
const chains = [
  ...
  {
    name: "G\xF6rli",
    title: "Ethereum Testnet G\xF6rli",
    chain: "ETH",
    network: "testnet",
    rpc: [
    ...
    nativeCurrency: { name: "G\xF6rli Ether", symbol: "ETH", decimals: 18 },
  ...
  },
  {
    ...
    title: "Arbitrum G\xF6rli Rollup Testnet",
    chainId: 421613,
    shortName: "arb-goerli",
    chain: "ETH",
    networkId: 421613,
    nativeCurrency: { name: "Arbitrum G\xF6rli Ether", symbol: "AGOR", decimals: 18 },
    rpc: ["https://goerli-rollup.arbitrum.io/rpc/"],
    faucets: [],
    infoURL: "https://arbitrum.io/",
    explorers: [
      {
        name: "Arbitrum G\xF6rli Rollup Explorer",
...
const createStore = () => {
  const { emit, get, subscribe: subscribe2, assign, deleteAll } = proxied();
  ...
  const accountsChangedHandler = (accounts) => switch1193Provider({
    addressOrIndex: Array.isArray(accounts) && accounts.length ? accounts[0] : 0
  });
  ...
  const set1193Provider = async (eipProvider, addressOrIndex, chainId) => {
    ...
    const provider2 = new ethers.providers.Web3Provider(eipProvider);
    assign({
      provider: provider2,
      eipProvider,
  ...
  const setProvider = async (provider2, addressOrIndex = 0) => {
    if (!provider2) {
    ...
    if (typeof provider2 === "object" && provider2.request)
      return set1193Provider(provider2, addressOrIndex);
    init();
    if (typeof provider2 !== "object" || !(Object.getPrototypeOf(provider2) instanceof ethers.providers.BaseProvider) && !(Object.getPrototypeOf(provider2) instanceof ethers.providers.UrlJsonRpcProvider)) {
      provider2 = new ethers.providers.JsonRpcProvider(provider2);
    }
    const { chainId } = await provider2.getNetwork();
    let signer, signerAddress2;
    if (addressOrIndex !== null) {
      try {
        if (typeof provider2.listAccounts === "function") {
          signer = provider2.getSigner(addressOrIndex);
        } else {
    ...
    assign({
      ...
      evmProviderType: provider2.constructor.name
    });
    emit();
  };
...
const makeEvmStores = (name) => {
  ...
  allStores[name].chainData = derived(
    evmStore,
    ($evmStore) => $evmStore.chainId ? getData(alwaysNumber($evmStore.chainId)) : {}
  );
  ...
  return new Proxy(allStores[name], {
    get: function(internal, property) {
      ...
      if (property === "attachContract")
        return registry.attachContract;
      if ([
        "setBrowserProvider",
        "setProvider",
        "disconnect",
...
const provider$1 = allStores.default.provider;
allStores.default.chainId;
allStores.default.signer;
...
let allowanceStore = writable();
const txMinada = writable();
const GWTContractAddress = readable("0x183254eaDCCb36779132eDD22e31e7B5500aEcE8");
const GWTContract = "0x183254eaDCCb36779132eDD22e31e7B5500aEcE8";
const USDCtContractAddress = readable("0x433E75eB0E582b8a4b9a4706977Ed90A85F671AA");
const USDCtContract = "0x433E75eB0E582b8a4b9a4706977Ed90A85F671AA";
let USDCtBalance = writable();
let GEWtBalance = writable();
const rpc$3 = "https://polygon-mumbai.g.alchemy.com/v2/j3qhn1OrOOjA1xLUKJbggXoC4mH_LFTu";
const chainid$3 = 80001;
const provider = new ethers.providers.JsonRpcProvider(rpc$3, chainid$3);
...
const rpc$2 = "https://polygon-mumbai.g.alchemy.com/v2/j3qhn1OrOOjA1xLUKJbggXoC4mH_LFTu";
const chainid$2 = 80001;
const CheckAllowance = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let $GWTContractAddress, $$unsubscribe_GWTContractAddress;
  let $signerAddress, $$unsubscribe_signerAddress;
  let $$unsubscribe_provider;
  let $$unsubscribe_txHash;
  let $$unsubscribe_contracts;
  let $USDCtAbi, $$unsubscribe_USDCtAbi;
  let $USDCtContractAddress, $$unsubscribe_USDCtContractAddress;
  let $allowanceStore, $$unsubscribe_allowanceStore;
  $$unsubscribe_GWTContractAddress = subscribe(GWTContractAddress, (value) => $GWTContractAddress = value);
  $$unsubscribe_signerAddress = subscribe(signerAddress, (value) => $signerAddress = value);
  $$unsubscribe_provider = subscribe(provider$1, (value) => value);
  $$unsubscribe_txHash = subscribe(txHash, (value) => value);
  $$unsubscribe_contracts = subscribe(contracts, (value) => value);
  $$unsubscribe_USDCtAbi = subscribe(USDCtAbi, (value) => $USDCtAbi = value);
  $$unsubscribe_USDCtContractAddress = subscribe(USDCtContractAddress, (value) => $USDCtContractAddress = value);
  $$unsubscribe_allowanceStore = subscribe(allowanceStore, (value) => $allowanceStore = value);
  const ethersProvider = new ethers.providers.JsonRpcProvider(rpc$2, chainid$2);
  new ethers.Contract($USDCtContractAddress, $USDCtAbi, ethersProvider);
  ethers.utils.parseEther("1");
  let { delayMs: delayMs2 } = $$props;
  if ($$props.delayMs === void 0 && $$bindings.delayMs && delayMs2 !== void 0)
    $$bindings.delayMs(delayMs2);
  $$unsubscribe_GWTContractAddress();
  $$unsubscribe_signerAddress();
  $$unsubscribe_provider();
  $$unsubscribe_txHash();
  $$unsubscribe_contracts();
  $$unsubscribe_USDCtAbi();
  $$unsubscribe_USDCtContractAddress();
  $$unsubscribe_allowanceStore();
  return `<main>${function(__value) {
    ...
    return function() {
      return `
		...
		${$allowanceStore == false ? `<div id="${"approveDiv"}"><h2>Apruebe el contrato por favor</h2>
					<button>Aprobar el contrato</button>
					<p>La primera vez hay que aprobar el contrato para
					</p>
					<p>poder interactuar con la blockchain
					</p></div>` : ``}
	`;
    }();
  }(getAllowance($signerAddress, $GWTContractAddress))}</main>`;
});
const rpc$1 = "https://polygon-mumbai.g.alchemy.com/v2/j3qhn1OrOOjA1xLUKJbggXoC4mH_LFTu";
const chainid$1 = 80001;
const USDCtBalance_1 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let $USDCtBalance, $$unsubscribe_USDCtBalance;
  let $signerAddress, $$unsubscribe_signerAddress;
  let $USDCtAbi, $$unsubscribe_USDCtAbi;
  let $USDCtContractAddress, $$unsubscribe_USDCtContractAddress;
  $$unsubscribe_USDCtBalance = subscribe(USDCtBalance, (value) => $USDCtBalance = value);
  $$unsubscribe_signerAddress = subscribe(signerAddress, (value) => $signerAddress = value);
  $$unsubscribe_USDCtAbi = subscribe(USDCtAbi, (value) => $USDCtAbi = value);
  $$unsubscribe_USDCtContractAddress = subscribe(USDCtContractAddress, (value) => $USDCtContractAddress = value);
  const provider2 = new ethers.providers.JsonRpcProvider(rpc$1, chainid$1);
  const contractReadOnly = new ethers.Contract($USDCtContractAddress, $USDCtAbi, provider2);
  async function USDCBalance() {
    ...
    set_store_value(USDCtBalance, $USDCtBalance = USDC, $USDCtBalance);
    return USDC / 1e18;
  }
  if ($$props.USDCBalance === void 0 && $$bindings.USDCBalance && USDCBalance !== void 0)
    $$bindings.USDCBalance(USDCBalance);
  $$unsubscribe_USDCtBalance();
  $$unsubscribe_signerAddress();
  $$unsubscribe_USDCtAbi();
  $$unsubscribe_USDCtContractAddress();
...
const chainid = 80001;
const GersteWeinTokenBalance = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let $GEWtBalance, $$unsubscribe_GEWtBalance;
  let $signerAddress, $$unsubscribe_signerAddress;
  let $GWTAbi, $$unsubscribe_GWTAbi;
  let $GWTContractAddress, $$unsubscribe_GWTContractAddress;
  $$unsubscribe_GEWtBalance = subscribe(GEWtBalance, (value) => $GEWtBalance = value);
  $$unsubscribe_signerAddress = subscribe(signerAddress, (value) => $signerAddress = value);
  $$unsubscribe_GWTAbi = subscribe(GWTAbi, (value) => $GWTAbi = value);
  $$unsubscribe_GWTContractAddress = subscribe(GWTContractAddress, (value) => $GWTContractAddress = value);
  const provider2 = new ethers.providers.JsonRpcProvider(rpc, chainid);
  const contractReadOnly = new ethers.Contract($GWTContractAddress, $GWTAbi, provider2);
  async function GersteWeinToken() {
    let GeWT = await contractReadOnly.balanceOf($signerAddress);
    GeWT = ethers.utils.formatEther(GeWT);
    GeWT = Number(GeWT).toFixed(2);
    set_store_value(GEWtBalance, $GEWtBalance = GeWT, $GEWtBalance);
    return GeWT / 1e18;
  }
  if ($$props.GersteWeinToken === void 0 && $$bindings.GersteWeinToken && GersteWeinToken !== void 0)
    $$bindings.GersteWeinToken(GersteWeinToken);
  $$unsubscribe_GEWtBalance();
  $$unsubscribe_signerAddress();
  $$unsubscribe_GWTAbi();
  $$unsubscribe_GWTContractAddress();
  return `${function(__value) {
...
`;
    ...
    return function() {
      return `
	<p>GersteWeinToken:</p>
	<p>${escape($GEWtBalance)}</p>
`;
    }();
  }(GersteWeinToken())}`;
});
const ConnectEthers = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.connectToDapp === void 0 && $$bindings.connectToDapp && connectToDapp !== void 0)
    $$bindings.connectToDapp(connectToDapp);
  $$unsubscribe_connected();
  $$unsubscribe_signerAddress();
  return `<main>${!$connected ? `<button>Conectar Metamask </button>` : `<button disabled>Metamask conectado </button>`}
	${!$connected ? `<p>Conectese a la aplicaci\xF3n por favor</p>` : `<div id="${"accountInfo"}"><p>Conectado con la cuenta :</p>
			<p>${escape($signerAddress)}</p></div>
		
		
		`}
</main>`;
});
const Balances = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let { delayMs: delayMs2 } = $$props;
  if ($$props.delayMs === void 0 && $$bindings.delayMs && delayMs2 !== void 0)
    $$bindings.delayMs(delayMs2);
  return `<main><div id="${"accountBalances"}" class="${"flex-container"}"><div><p>Su saldos son:
				<br></p></div>
		
		<div><p>${validate_component(USDCtBalance_1, "UsdCtBalance").$$render($$result, {}, {}, {})}</p></div>
		<div><p>${validate_component(GersteWeinTokenBalance, "GeWT").$$render($$result, {}, {}, {})}</p></div>
		</div></main>`;
});
const TxHashComponent = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let $txMinada, $$unsubscribe_txMinada;
  let $provider, $$unsubscribe_provider;
  let $txHash, $$unsubscribe_txHash;
  $$unsubscribe_txMinada = subscribe(txMinada, (value) => $txMinada = value);
  $$unsubscribe_provider = subscribe(provider$1, (value) => $provider = value);
  $$unsubscribe_txHash = subscribe(txHash, (value) => $txHash = value);
  let { delayMs: delayMs2 } = $$props;
  function resetTxHash() {
    setTimeout(
      () => {
        set_store_value(txHash, $txHash = void 0, $txHash);
  ...
  const txreceipt = async () => {
    ...
    console.log(txWait);
    set_store_value(txMinada, $txMinada = true, $txMinada);
    console.log("Ejecutando reseteo de $txHash");
    resetTxHash();
    return "Transacci\xF3n minada!";
  };
  if ($$props.delayMs === void 0 && $$bindings.delayMs && delayMs2 !== void 0)
    $$bindings.delayMs(delayMs2);
  $$unsubscribe_txMinada();
  $$unsubscribe_provider();
  $$unsubscribe_txHash();
  return `<main>${$txHash != void 0 ? `
		<br>
			<div id="${"txhash"}"><p>Transacci\xF3n mandada:</p>
				<a href="${"https://mumbai.polygonscan.com/tx/" + escape($txHash, true)}" target="${"_blank"}">${escape($txHash)}</a></div>
			${function(__value) {
    if (is_promise(__value)) {
      __value.then(null, noop);
      return `
				<p>Esperando que la transacci\xF3n se confirme...
				</p>
			`;
    }
...
const BuyGersteToken_v3 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let $GWTContractAddress, $$unsubscribe_GWTContractAddress;
  let $signerAddress, $$unsubscribe_signerAddress;
  let $provider, $$unsubscribe_provider;
  let $txHash, $$unsubscribe_txHash;
  ...
  $$unsubscribe_connected = subscribe(connected, (value) => $connected = value);
  let amountToBuy = 0;
  let { mintTokencitoV2 = async () => {
    var _a;
    try {
      const amountTokens = (_a = document.getElementById("amountToBuy")) == null ? void 0 : _a.value;
      console.log(amountTokens);
      if (amountTokens == 0) {
        console.log("Elija un importe v\xE1lido");
      } else {
        ...
        set_store_value(txHash, $txHash = txReceipt, $txHash);
  ...
  if ($$props.mintTokencitoV2 === void 0 && $$bindings.mintTokencitoV2 && mintTokencitoV2 !== void 0)
    $$bindings.mintTokencitoV2(mintTokencitoV2);
  $$unsubscribe_GWTContractAddress();
  $$unsubscribe_signerAddress();
  $$unsubscribe_provider();
  $$unsubscribe_txHash();
  $$unsubscribe_contracts();
  $$unsubscribe_connected();
  return `<main>${!$connected ? `<p></p>` : `<div>${$signerAddress === void 0 ? `<input type="${"number"}" id="${"amountToBuy"}" min="${"0"}" max="${"999"}" value="${"0"}" disabled>

				<button disabled>Comprar GersteWeinToken</button>` : `<h1>${escape(amountToBuy)}</h1>
				<input type="${"number"}" id="${"amountToBuy"}" min="${"0"}" max="${"999"}"${add_attribute("value", amountToBuy, 0)}>
				<br><br>
				<button>Comprar GersteWeinToken</button>
				<br>`}</div>`}</main>`;
});
const BurnGersteToken = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let $GWTContractAddress, $$unsubscribe_GWTContractAddress;
  let $signerAddress, $$unsubscribe_signerAddress;
  let $provider, $$unsubscribe_provider;
  let $txHash, $$unsubscribe_txHash;
  ...
  $$unsubscribe_connected = subscribe(connected, (value) => $connected = value);
  let amountToBurn = 0;
  let { burnTokencito = async () => {
    var _a;
    try {
      const amountTokens = (_a = document.getElementById("amountToBurn")) == null ? void 0 : _a.value;
      console.log(amountTokens);
      if (amountTokens == 0) {
        console.log("Elija un importe v\xE1lido");
        return false;
  ...
  if ($$props.burnTokencito === void 0 && $$bindings.burnTokencito && burnTokencito !== void 0)
    $$bindings.burnTokencito(burnTokencito);
  $$unsubscribe_GWTContractAddress();
  $$unsubscribe_signerAddress();
  $$unsubscribe_provider();
  $$unsubscribe_txHash();
  $$unsubscribe_contracts();
  $$unsubscribe_connected();
  return `<main>${!$connected ? `<p></p>` : `<div>${$signerAddress === void 0 ? `<input type="${"number"}" id="${"amountToBurn"}" min="${"0"}" max="${"999"}" value="${"0"}" disabled>

				<button disabled>Vender GersteWeinToken</button>` : `<h1>${escape(amountToBurn)}</h1>
				<input type="${"number"}" id="${"amountToBurn"}" min="${"0"}" max="${"999"}"${add_attribute("value", amountToBurn, 0)}>
				<br><br>
				<button>Vender GersteWeinToken</button>`}</div>`}</main>`;
});
const _page_svelte_svelte_type_style_lang = "";
const css = {
  code: "body{background-color:rgb(27, 27, 27);font-family:sans-serif;color:white;text-align:center}h1.svelte-ym9zm8{color:white;font-family:sans-serif;text-align:center}",
  map: null
};
let delayMs = 1200;
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_connected();
  $$unsubscribe_allowanceStore();
  return `<head><title>GersteWein Tokens !</title></head>

<main><h1 class="${"svelte-ym9zm8"}">GersteWein Tokens</h1>

	

	${validate_component(ConnectEthers, "ConnectEthers").$$render($$result, {}, {}, {})}
	${$connected ? `${validate_component(Balances, "Balances").$$render($$result, { delayMs }, {}, {})}

		
		${validate_component(CheckAllowance, "CheckAllowance").$$render($$result, { delayMs }, {}, {})}
		${$allowanceStore == true ? `<div id="${"buying"}">${validate_component(BuyGersteToken_v3, "BuyGersteToken").$$render($$result, {}, {}, {})}</div>` : ``}
		
		<div id="${"selling"}">${validate_component(BurnGersteToken, "BurnGersteToken").$$render($$result, {}, {}, {})}</div>

		${validate_component(TxHashComponent, "TxHashComponent").$$render($$result, { delayMs }, {}, {})}` : ``}
</main>`;
});
export {
  Page as default
